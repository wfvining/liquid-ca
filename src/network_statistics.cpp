...
void network_statistics(int num_iterations, double speed)
{
   ...
   for(int iteration = 0; iteration < num_iterations; iteration++)
   {
      ...
      for(int step = 0; step < 5000; step++)
      {
         ...
         for(int i = 0; i < model_config.num_agents; i++)
         {
            ...
            degree_distribution[degree] = degree_distribution[degree] + 1.0;
         }
      }
   ...
   {
      std::cout << d << " " << degree_distribution[d]/(num_iterations*5000.0*model_config.num_agents) << std::endl;
      expected_degree += (degree_distribution[d]/(num_iterations*5000.0*model_config.num_agents))*d;
   }
   ...
   for(int d = 0; d < degree_distribution.size(); d++)
   {
      variance += pow(d - expected_degree, 2) * (degree_distribution[d] / (num_iterations * 5000.0*model_config.num_agents));
   }
